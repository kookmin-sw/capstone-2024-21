...
public class Interact : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isInvetigating)
        {
            ...
            {
                //수색을 성공적으로 마쳤다면 스폰
                if(selectedTarget.gameObject.GetComponent<BatterySpawner>())
                {
                    selectedTarget.GetComponent<BatterySpawner>().SpawnItem();
                }
                else if (selectedTarget.gameObject.GetComponent<WeaponSpawner>())
                {
                    selectedTarget.GetComponent<WeaponSpawner>().SpawnItem();
                }
                else if (selectedTarget.gameObject.GetComponent<ItemSpawner>())
                {
